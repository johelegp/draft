...
(\tcode{m} denotes a value of type \tcode{L},
\tcode{rel_­time} denotes a value of a specialization of duration, and
\tcode{abs_­time} denotes a value of a specialization of \tcode{time_­point}).
...
the relative timeout\iref{thread.req.timing} specified by \tcode{rel_­time}.
The function will not return within the timeout specified by \tcode{rel_­time}
...
the absolute timeout\iref{thread.req.timing} specified by \tcode{abs_­time}.
The function will not return before the timeout specified by \tcode{abs_­time}
unless it has obtained a lock on \tcode{m} for the current execution agent.
If an exception is thrown then a lock has not been acquired for
the current execution agent.
